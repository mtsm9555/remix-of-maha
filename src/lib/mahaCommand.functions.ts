...
import { MahaError, mapGatewayStatus, safeHandler } from "./errors";
...
export const transcribeMaha = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) =>
    safeHandler("transcribeMaha", async () => {
      const key = process.env.LOVABLE_API_KEY;
      if (!key) throw new MahaError("server_error", "AI service is not configured.");
      ...
      if (!res.ok) {
        ...
        throw mapGatewayStatus(res.status, err);
      }
...
export const analyzeImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) =>
    safeHandler("analyzeImage", async () => {
      const key = process.env.LOVABLE_API_KEY;
      if (!key) throw new MahaError("server_error", "AI service is not configured.");
      const gateway = createLovableAiGatewayProvider(key);
