...
function OSPage() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      coreRef.current.style.boxShadow = `0 0 30px #00eaff, 0 0 ${glow + 50}px #00eaff`;
    }, 1200);
    return () => clearInterval(t);
  }, []);
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      if (pick) {
        pick.style.background = "rgba(0,234,255,0.12)";
        pick.style.borderColor = "rgba(0,234,255,0.4)";
      }
    }, 1500);
    return () => clearInterval(t);
  }, []);
