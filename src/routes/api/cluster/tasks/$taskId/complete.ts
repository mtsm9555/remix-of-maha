...
export const Route = createFileRoute("/api/cluster/tasks/$taskId/complete")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        const body = await request.json().catch(() => ({} as { result?: unknown; error?: string }));
        const { TaskDistributionEngine } = await import("@/backend/infrastructure/cluster/TaskDistributionEngine.server");
        if (body.error) await TaskDistributionEngine.failTask(params.taskId, body.error);
        else await TaskDistributionEngine.completeTask(params.taskId, body.result ?? null);
